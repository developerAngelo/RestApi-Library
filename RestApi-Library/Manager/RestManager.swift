...
class RestManager{
    ...
    var urlQueryParameter = RestEntity()
    ...
    private func addURLQueryParameters(toURL url: URL) -> URL{
        if urlQueryParameter.totalItems() > 0{
            guard var urlComponents = URLComponents(url: url, resolvingAgainstBaseURL: false) else {return url}
            
            var queryItems = [URLQueryItem]()
            for (key, value) in urlQueryParameter.allValues() {
    ...
    func makeRequest(toURL url: URL, with httpMethod: HttpMethod, completion: @escaping (_ result: Results) -> Void){
    ...
    func getData(fromURL url: URL, completion: @escaping(_ data: Data?) -> Void){
        DispatchQueue.global(qos: .userInitiated).async {
            ...
            let session = URLSession(configuration: sesssionConfiguration)
